...
| `home`/`listings`/`eligibility`/`mypage` 구현 | `pinhouse-frontend`, `code-review-guard` | `vercel-react-best-practices` | G1, G2, G4, G5 | 화면/도메인 unit, `typecheck` |
| React Query 캐시/무효화 | `pinhouse-frontend`, `code-review-guard` | `vercel-react-best-practices` | G1, G2, G4, G5 | query key/invalidation unit, `typecheck` |
| BFF/API route 변경 | `pinhouse-frontend`, `code-review-guard` | 없음 | G1, G2, G3, G4, G5 | route handler unit, 실패 응답/쿠키 분기 확인 |
| 쿠키/세션/민감정보 처리 | `pinhouse-frontend`, `code-review-guard` | 없음 | G2, G3, G4, G5 | cookie 옵션, console/localStorage 노출 확인 |
...
| CI/CD 워크플로우 | `pinhouse-frontend`, `code-review-guard` | 없음 | G1, G3, G4, G5 | YAML diff, 트리거/secret/branch 조건 확인 |
...
- `pinhouse-frontend` 작업은 필요한 `references/*.md`만 추가로 읽는다.
