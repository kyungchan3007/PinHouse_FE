...
interface EligibilityPageClientProps {
  initialLatest: DiagnosisLatestData | null;
  isEligibilityOnly: boolean;
}

export function EligibilityPageClient({
  initialLatest,
  isEligibilityOnly,
}: EligibilityPageClientProps) {
  ...
  useEffect(() => {
    ...
    const applyLatest = (data: DiagnosisLatestData) => {
      setDiagnosisResult(
        {
  ...
  }, [initialLatest, isEligibilityOnly, reset, setDiagnosisResult]);
