import { SVGProps } from "react";

export const InfoButoonImg = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg width="68" height="68" viewBox="0 0 68 68" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g clip-path="url(#clip0_9776_42469)">
        <path
          d="M29.2631 47.8681L56.2389 40.6129L47.8695 9.26103C47.4492 7.68692 45.8373 6.75277 44.269 7.17456L20.1328 13.666L29.2631 47.8681Z"
          fill="url(#paint0_linear_9776_42469)"
        />
        <path
          d="M26.8047 50.0566L56.62 42.0378L57.3809 44.888L27.5655 52.9068L26.8047 50.0566Z"
          fill="#F2F5FF"
        />
        <path
          d="M28.4234 19.073C28.2133 18.286 28.6786 17.477 29.4627 17.2661L36.5616 15.3568C37.3458 15.146 38.1517 15.613 38.3618 16.4001L39.1227 19.2503C39.3328 20.0373 38.8675 20.8463 38.0834 21.0572L30.9845 22.9664C30.2003 23.1773 29.3944 22.7103 29.1843 21.9232L28.4234 19.073Z"
          fill="white"
        />
        <path
          d="M18.0027 14.2387C16.8265 14.5551 16.1285 15.7686 16.4437 16.9492L25.7642 51.8637L27.184 51.4819C26.9739 50.6948 27.4392 49.8858 28.2233 49.675L29.6431 49.2931L20.1324 13.666L18.0027 14.2387Z"
          fill="url(#paint1_linear_9776_42469)"
        />
        <path
          d="M27.1336 48.4409C25.9574 48.7572 25.2594 49.9707 25.5745 51.1513L25.955 52.5764C26.2701 53.757 27.4791 54.4576 28.6553 54.1412L55.6311 46.8861C56.5583 46.6367 57.1884 45.8297 57.2591 44.9208L28.9847 52.5252C28.2006 52.7361 27.3946 52.269 27.1845 51.482C26.9744 50.6949 27.4398 49.8859 28.2239 49.6751L56.6195 42.038L56.239 40.613L27.1336 48.4409Z"
          fill="url(#paint2_linear_9776_42469)"
        />
        <path
          d="M41.4297 7.93799L42.8495 7.55614L53.5015 47.4585L52.0817 47.8404L41.4297 7.93799Z"
          fill="url(#paint3_linear_9776_42469)"
        />
      </g>
      <g clip-path="url(#clip1_9776_42469)">
        <path
          d="M12.2889 60.1603C12.6048 60.1603 12.9201 60.04 13.1614 59.7988L18.703 54.2573C19.1849 53.7754 19.1849 52.9942 18.703 52.5123C18.2211 52.0304 17.4399 52.0304 16.958 52.5123L11.4164 58.0538C10.9345 58.5357 10.9345 59.3169 11.4164 59.7988C11.6577 60.04 11.973 60.1603 12.2889 60.1603Z"
          fill="url(#paint4_linear_9776_42469)"
        />
        <foreignObject x="12.9147" y="35.7609" width="22.5398" height="22.5398">
          <div
            style={{
              backdropFilter: "blur(1.17px)",
              clipPath: "url(#bgblur_2_9776_42469_clip_path)",
              height: "100%",
              width: "100%",
            }}
          />
        </foreignObject>
        <path
          data-figma-bg-blur-radius="2.33143"
          d="M24.1855 38.0923C19.2564 38.0923 15.2464 42.1024 15.2461 47.0308C15.2461 49.4943 16.2494 51.7286 17.8672 53.3472L17.8662 53.3481C19.4848 54.9666 21.7195 55.9691 24.1836 55.9692C29.1122 55.9692 33.123 51.9584 33.123 47.0298C33.123 44.5657 32.1205 42.331 30.502 40.7124C28.8834 39.0944 26.6492 38.0924 24.1855 38.0923Z"
          fill="white"
          fill-opacity="0.6"
        />
        <path
          d="M24.1859 57.0581C29.7147 57.0581 34.2129 52.56 34.2129 47.0311C34.2129 41.5023 29.7147 37.0034 24.1859 37.0034C18.657 37.0034 14.1582 41.5016 14.1582 47.0311C14.1582 52.5607 18.6563 57.0581 24.1859 57.0581ZM24.1859 39.181C28.5142 39.181 32.0353 42.7029 32.0353 47.0311C32.0353 51.3594 28.5135 54.8806 24.1859 54.8806C19.8583 54.8806 16.3357 51.3587 16.3357 47.0311C16.3357 42.7036 19.8576 39.181 24.1859 39.181Z"
          fill="url(#paint5_linear_9776_42469)"
        />
      </g>
      <defs>
        <clipPath id="bgblur_2_9776_42469_clip_path" transform="translate(-12.9147 -35.7609)">
          <path d="M24.1855 38.0923C19.2564 38.0923 15.2464 42.1024 15.2461 47.0308C15.2461 49.4943 16.2494 51.7286 17.8672 53.3472L17.8662 53.3481C19.4848 54.9666 21.7195 55.9691 24.1836 55.9692C29.1122 55.9692 33.123 51.9584 33.123 47.0298C33.123 44.5657 32.1205 42.331 30.502 40.7124C28.8834 39.0944 26.6492 38.0924 24.1855 38.0923Z" />
        </clipPath>
        <linearGradient
          id="paint0_linear_9776_42469"
          x1="33.6207"
          y1="10.0384"
          x2="42.8148"
          y2="44.2233"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#6084FF" />
          <stop offset="1" stop-color="#9FB5FF" />
        </linearGradient>
        <linearGradient
          id="paint1_linear_9776_42469"
          x1="28.223"
          y1="52.5241"
          x2="18.0636"
          y2="14.7498"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#8EA8FF" />
          <stop offset="1" stop-color="#3E6AFE" />
        </linearGradient>
        <linearGradient
          id="paint2_linear_9776_42469"
          x1="38.371"
          y1="38.0072"
          x2="42.1665"
          y2="50.5066"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#91B0FF" />
          <stop offset="1" stop-color="#E9EEFF" />
        </linearGradient>
        <linearGradient
          id="paint3_linear_9776_42469"
          x1="42.1396"
          y1="7.74706"
          x2="52.866"
          y2="47.6294"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#91B0FF" />
          <stop offset="1" stop-color="#E9EEFF" />
        </linearGradient>
        <linearGradient
          id="paint4_linear_9776_42469"
          x1="15.2045"
          y1="52.361"
          x2="13.9478"
          y2="59.8982"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#F59E0B" />
          <stop offset="1" stop-color="#FFBB18" />
        </linearGradient>
        <linearGradient
          id="paint5_linear_9776_42469"
          x1="29.8405"
          y1="40.408"
          x2="17.1207"
          y2="54.0956"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#FFBA18" />
          <stop offset="1" stop-color="#FFD95B" />
        </linearGradient>
        <clipPath id="clip0_9776_42469">
          <rect
            width="47.0474"
            height="47.1997"
            fill="white"
            transform="matrix(0.965684 -0.259721 0.257919 0.966166 8.01367 13.8706)"
          />
        </clipPath>
        <clipPath id="clip1_9776_42469">
          <rect
            width="29.0343"
            height="29.0343"
            fill="white"
            transform="matrix(-1 0 0 1 37.0488 34)"
          />
        </clipPath>
      </defs>
    </svg>
  );
};
